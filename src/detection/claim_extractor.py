...
class ClaimExtractor:
    ...
    def _clean_claim(text):
        ...
        # Remove dangling list markers without deleting factual years such as 2001 or 2020.
        text = re.sub(r"\s+(?:[1-9]|1[0-9]|20)[\).]\s*$", "", text)
    ...
    def _is_meta_or_non_factual(text):
        ...
        # Drop headings/questions. These are prompts or section titles, not factual claims.
        heading_exact = {
            "overview", "introduction", "background", "key points", "benefits",
            "applications", "limitations", "examples", "conclusion",
        }
        if lower.strip(":") in heading_exact:
            return True
        if lower.endswith("?") and re.match(r"^(what|who|when|where|why|how|explain|define)\b", lower):
            return True
        if re.match(r"^(what is|what are|who is|explain|define)\b", lower) and len(lower.split()) <= 8:
            return True
